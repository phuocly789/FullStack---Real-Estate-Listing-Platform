...
const PropertyForm = ({ initialValues = {}, onSubmit }) => {
    ...
    const initializeLocationFields = async (location) => {
        ...
        try {
            ...
            // Cập nhật province và lấy danh sách quận/huyện
            ...
            // Cập nhật district và lấy danh sách phường/xã
            ...
            // Cập nhật ward và location
            setWards(districtData.wards || []);
            setForm(prev => ({ ...prev, ward: ward.code.toString(), location }));
        } catch (error) {
    ...
    useEffect(() => {
        if (initialValues && initialValues.longitude && initialValues.latitude) {
            setForm(prev => ({
                ...prev,
    ...
    useEffect(() => {
        fetch('https://provinces.open-api.vn/api/p/')
            ...
            .then((data) => {
                setProvinces(data);
                // Khởi tạo province, district, ward nếu có initialValues.location
    ...
    useEffect(() => {
        if (form.province && !initialValues.location) {
            setLoading(true);
    ...
    useEffect(() => {
        if (form.district && !initialValues.location) {
            setLoading(true);
    ...
    // Cập nhật location khi province, district, ward thay đổi
    useEffect(() => {
        if (form.province && form.district && form.ward && wards.length > 0) {
            const ward = wards.find(w => w.code === parseInt(form.ward));
            if (!ward) return; // Đảm bảo ward thuộc district hiện tại
    ...
    // Lấy kinh độ/vĩ độ khi chọn đầy đủ địa chỉ
    useEffect(() => {
        if (form.province && form.district && form.ward && wards.length > 0 && !initialValues.longitude && !initialValues.latitude) {
            const ward = wards.find(w => w.code === parseInt(form.ward));
            if (!ward) return; // Đảm bảo ward thuộc district hiện tại
            setLoading(true);
            const province = provinces.find(p => p.code === parseInt(form.province))?.name || '';
            const district = districts.find(d => d.code === parseInt(form.district))?.name || '';
            const wardName = ward.name || '';
            let address = `${wardName}, ${district}, ${province}, Vietnam`;

            fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}`, {
                headers: {
                    'User-Agent': 'PropertyFormApp/1.0 (your-email@example.com)'
                }
            })
                .then((response) => response.json())
                .then((data) => {
                    if (data.length > 0) {
                        ...
                        // Thử địa chỉ cấp quận/huyện
                        address = `${district}, ${province}, Vietnam`;
                        fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}`, {
                            headers: {
                                'User-Agent': 'PropertyFormApp/1.0 (your-email@example.com)'
                            }
                            ...
                            .then((data) => {
                                if (data.length > 0) {
                                    ...
                                    setErrors(prev => ({
                                        ...prev,
                                        api: `Không thể tìm kinh độ/vĩ độ cho địa chỉ: ${address}`
                                    }));
                                }
                                setLoading(false);
                            })
                            .catch(() => {
                                setErrors(prev => ({
                                    ...prev,
                                    api: `Lỗi khi lấy kinh độ/vĩ độ cho địa chỉ: ${address}`
                                }));
                                setLoading(false);
                            });
                    }
    ...
    const validate = () => {
        ...
        if (!form.longitude || form.longitude < -180 || form.longitude > 180) newErrors.longitude = 'Kinh độ không hợp lệ';
    ...
    const handleSubmit = (e) => {
        ...
        onSubmit(parsedForm);
        setTimeout(() => setIsSubmitting(false), 1000); // Reset submit state after 1s
    };
